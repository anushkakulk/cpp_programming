...
template <class T>
CommaSeparatedList<T>::CommaSeparatedList(T *d, int s) // FIX: should take in pointer d
{
   data = d;
   size = s;
}
template <class T>
    void CommaSeparatedList<T>::printList() // FIX: fixed improper syntax
{
   cout << "Comma separated list:" << endl; // FIX: <<. not >>
   for (int x = 0; x < size; ++x)
   {
      cout << data[x]; // FIX: data[x] not [y]
      if (x != size - 1) {
         cout << ", "; // FIX: prints the comma after all elems besides the last
      }
   }
   cout << endl << endl;
}
...
ostream &operator<<(ostream &out, const Book &b)
{
   ...
   return out; // FIX: return out, not os
}
...
int main()
{
   ...
   CommaSeparatedListSize = sizeof(someBook) / sizeof(someBook[0]); // FIX: sizeof , not sizeif
...
/*
FIXES
- ensured constructor arg matched definition with pointers
- implemented the comma separation and fixed any syntax/logic errors in the loop
- fixed Books's << overload to return out, not os
- fixed syntactical/logic errors throughout
*/